...
class roachInterface(object):
    ...
    def makeFreqComb(self, min_neg, max_neg, min_pos, max_pos, offset, nfreqs):
        
        self.min_neg_freq = min_neg
        self.max_neg_freq = max_neg
        self.min_pos_freq = min_pos
        self.max_pos_freq = max_pos
        self.symm_offset = offset
        self.Nfreq = nfreqs

        __, self.pos_delta = np.linspace(self.min_pos_freq, self.max_pos_freq, self.Nfreq, retstep = True)
        __, self.neg_delta = np.linspace(self.min_neg_freq + self.symm_offset, self.max_neg_freq + self.symm_offset, self.Nfreq, retstep = True)
    ...
    def read_mixer_snaps(self, chan, mixer_out = True, fir = True):
        ...
        if fir:
            self.fpga.write_int(self.regs['fir_snap_ctrl_reg'], 0)
            self.fpga.write_int(self.regs['fir_snap_ctrl_reg'], 1)
        self.fpga.write_int(self.regs['DDC_fftbin_ctrl_reg'], 1)
        self.fpga.write_int(self.regs['DDC_mixerout_ctrl_reg'], 1)
        mixer_in = np.fromstring(self.fpga.read(self.regs['DDC_fftbin_bram_reg'], 16*2**14),dtype='>i2').astype('float')
        if mixer_out:
            mixer_out = np.fromstring(self.fpga.read(self.regs['DDC_mixerout_bram_reg'], 8*2**14),dtype='>i2').astype('float')
            if fir:
                lpf_out = np.fromstring(self.fpga.read(self.regs['DDC_fftbin_bram_reg'], 8*2**14),dtype='>i2').astype('float')
            else:
                lpf_out = []
    ...
    # returns snap data for the dds mixer inputs and outputs
        self.fpga.write_int(self.regs['dds_shift'], shift)
        if (chan % 2) > 0: # if chan is odd
            self.fpga.write_int(self.regs['DDC_chan_sel_reg'], (chan - 1) / 2)
        else:
            self.fpga.write_int(self.regs['DDC_chan_sel_reg'], chan/2)
        ...
        self.fpga.write_int(self.regs['DDC_fftbin_ctrl_reg'], 1)
        self.fpga.write_int(self.regs['DDC_mixerout_ctrl_reg'], 1)
        mixer_in = np.fromstring(self.fpga.read(self.regs['DDC_fftbin_bram_reg'], 16*2**10),dtype='>i2').astype('float')
        if mixer_out:
            mixer_out = np.fromstring(self.fpga.read(self.regs['DDC_mixerout_bram_reg'], 8*2**10),dtype='>i2').astype('float')
    ...
    def read_accum_snap(self):
        ...
        self.fpga.write_int(self.regs['accum_snap_ctrl_reg'], 0)
        self.fpga.write_int(self.regs['accum_snap_ctrl_reg'], 1)
        accum_data = np.fromstring(self.fpga.read(self.regs['accum_snap_bram_reg'], 16*2**9), dtype = '>i').astype('float')
        I = accum_data[0::2]
        Q = accum_data[1::2]
        return I, Q
    ...
    def plotADC(self):
        ...
        while count < stop:
            ...
            time.sleep(0.1)
            adc = (np.fromstring(self.fpga.read(self.regs['adc_snap_bram_reg'],(2**10)*8),dtype='>h')).astype('float')
            adc /= (2**15)
            adc *= 550.
    ...
    def plotFFT(self):
        ...
        stop = 1.0e6
        while(count < stop):
            self.fpga.write_int(self.regs['fft_snap_ctrl_reg'],0)
            self.fpga.write_int(self.regs['fft_snap_ctrl_reg'],1)
            fft_snap = (np.fromstring(self.fpga.read(self.regs['fft_snap_bram_reg'],(2**9)*8),dtype='>i2')).astype('float')
            I0 = fft_snap[0::4]
            Q0 = fft_snap[1::4]
            I1 = fft_snap[2::4]
            Q1 = fft_snap[3::4]
