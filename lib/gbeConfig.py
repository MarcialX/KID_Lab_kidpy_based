...
#            self.s.setsockopt(sock.SOL_SOCKET, sock.SO_RCVBUF, self.buf_size)
            ...
            self.s.bind((self.gc['udp_dest_device'], 3))
    ...
    def parsePacketData(self):
        ...
        if not packet:
            print "parsePacketData: Non-Roach packet received"
            return
        ...
        ### Filter on source IP ###
        if (saddr != self.gc['udp_src_ip']):
            #print "parsePacketData: saddr !='udp_src_ip': Non-Roach packet received"
            return
