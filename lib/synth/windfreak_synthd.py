...
class windfreak_synthd(object):
    ...
    def getPLLPower(self):
        return bool(int(self.sendCommand('E?')))
    def setPLLPower(self,value):
        assert value in (True,False)
    ...
    def getSweepLookupTable(self):
        return (self.sendCommand('L?'))
    def setSweepLookupTable(self,freqs,powers):
        assert len(freqs)==100
        assert len(powers)==100
        cmd = ''
        for j in range(100):
            cmd+='L%df%.8f'%(j,freqs[j]/1e6)
            cmd+='L%da%.3f'%(j,powers[j])
        self.sendCommand(cmd)
    def deleteSweepLookupTable(self):
        self.sendCommand('Ld')
    ...
    def setAMStepTime(self,value):
        assert value>=0 and value<=65535e-6
...
# Program Frequency Sweep table? and FM table?
    ...
    def setPLLReferenceFrequency(self,value):
        assert value>=10e6 and value<=110e6
